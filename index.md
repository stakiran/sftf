...
[日本語](index.md) | [English](index_EN.md) | [GitHub](https://github.com/stakiran/sftf)

sftf とは単一の機能を持つテキストファイルを指します。私は[Thymer](https://thymer.com/)を見たとき、この概念を思いつきました。いくつかの観点で例やフレームワークを揃えたので、ぜひご活用ください。あなたの人生に幸あれ！
...
from GTD, [GTDを噛み砕く](https://stakiran.github.io/gtd_kamikudaku/)に基づいて:
...
## 生成AIによる例
OpenAI API を使用。プロンプトは[こちら（日本語）](prompt_fot_sftf.md)。
...
## SFTF フレームワーク ～キラノ分類～
- SFTF を比較的高精度に分類できるものです
    - 1 つのシステムは 4 つのカテゴリーを持ち、これをシステムモジュールと呼びます
    - キラノ分類では 5 つのシステムモジュールをつくっています
        - 記録系、適用系、駆動系、行動系、処理系
- あなたオリジナルのシステムモジュールをつくっても構いませんが、（キラノ分類に従うなら）1モジュール4カテゴリーの単位でつくっていきましょう
    - 4 という数字は理解のしやすさ、使いやすさ、分類の精度などをバランスよく取れる数です
